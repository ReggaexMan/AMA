...
class SecurityController extends AbstractController
{
    ...
    public function signup(UserAuthenticatorInterface $userAuthenticator, Request $request, EntityManagerInterface $em, UserPasswordHasherInterface $passwordHasher, MailerInterface $mailer): Response
    {
        ...
        if ($userForm->isSubmitted() && $userForm->isValid()) {
            ...
            // à l'inscription, tout utilisateur aura le ROLE_USER de base
            $user->setRoles(['ROLE_USER']);

            $em->persist($user);
            $em->flush();
            $this->addFlash('success', 'Bienvenue sur AMA !');

            $email = new TemplatedEmail();
            $email->to($user->getEmail())
                ->subject('Bienvenue sur AMA')
    ...
    public function resetPasswordRequest(MailerInterface $mailer, Request $request, UserRepository $userRepository, ResetPasswordRepository $resetPasswordRepository, EntityManagerInterface $em)
    {
        ...
        if ($emailForm->isSubmitted() && $emailForm->isValid()) {
            ...
            if ($user) {
                ...
                $token = substr(str_replace(['+', '/', '='], '', base64_encode(random_bytes(30))), 0, 20); //La fonction str_replace() permet d'enlever les caractères qui ne sont pas utilisables dans une URL.
